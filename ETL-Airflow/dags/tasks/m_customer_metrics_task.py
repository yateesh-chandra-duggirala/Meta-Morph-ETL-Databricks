...
from pyspark.sql.functions import (
    ...
    current_timestamp, desc, asc, count, row_number,
    max, min
)
...
def customer_metrics_upsert():
    ...
    logging.info("Data Frame : 'JNR_Full' is built...")
    ...
    AGG_TRANS = JNR_Full \
        ...
        .agg(
            ...
            coalesce(
                sum("shipping_cost"),
                lit(0)
            ).alias("agg_TOTAL_SHIPPING_COST"),
            coalesce(
                sum(col("quantity") * col("selling_price")), lit(0)
            ).alias("agg_EXPENDITURE"),
            coalesce(
                sum(
                    col(
                        "quantity"
                    ) * col("selling_price") * col("discount") / lit(100)
                ), lit(0)
            ).alias("agg_TOTAL_AMOUNT_SAVINGS"),
            sum(
                when(col("order_status") == 'Delivered', lit(1))
                .otherwise(lit(0))
            ).alias("agg_DELIVERED_ORDERS_COUNT"),
            sum(
                when(col("order_status") == 'Cancelled', lit(1))
                .otherwise(lit(0))
            ).alias("agg_CANCELLED_ORDERS_COUNT"),
        )\
        .withColumn(
            "AVERAGE_ORDER_VALUE",
            coalesce(
                col("agg_EXPENDITURE") / col("agg_TOTAL_ORDERS"), lit(0)
            )
        ) \
        .withColumn(
            "ACTIVE_CUSTOMER_FLAG",
            when(
                col("agg_LAST_PURCHASE_DATE") >= current_date() - 4,
                lit("TRUE")
            )
            .otherwise(lit("FALSE"))
        ) \
    ...
    JNR_ALL = AGG_TRANS.alias("agg") \
        ...
        .select(
            ...
            coalesce(
                col("agg.agg_TOTAL_ORDERS"), lit(0)
            ).alias("TOTAL_ORDERS"),
            round(
                col("agg.agg_TOTAL_AMOUNT_SAVINGS"), 2
            ).alias("TOTAL_AMOUNT_SAVINGS"),
            round(
                col("agg.agg_TOTAL_SHIPPING_COST"), 2
            ).alias("TOTAL_SHIPPING_COST"),
            round(
                col("agg.agg_EXPENDITURE"), 2
            ).alias("EXPENDITURE"),
            round(
                col("agg.AVERAGE_ORDER_VALUE"), 2
            ...
            col(
                "agg.agg_DELIVERED_ORDERS_COUNT"
            ).alias("DELIVERED_ORDERS_COUNT"),
            col(
                "agg.agg_CANCELLED_ORDERS_COUNT"
    ...
    logging.info("Data Frame : 'Shortcut_To_Customer_Metrics' is built...")
    ...
    return "customer_metrics data ingested successfully!"
